...
module.exports.getOffline_table=function(req, res){
	try{
		...
		form.parse(req, function(err, fields, files) {
			...
			if(!city)city='';
			let yearFrom=Number(fields.yearFrom);
			let yearTo=Number(fields.yearTo);
			let regExpCity = new RegExp(city, 'i');
			let regExpBk=new RegExp(bk, 'i');
			var data={};
			async.parallel([
				(callback)=>{
					BkPPS.find({bk:{$regex:regExpBk}, address:{$regex:regExpCity}, year:yearFrom}, (err, rep)=>{
						if(rep.length>0) data.yearFromQuantity=rep.length;
						else data.yearFromQuantity='N/A';
				...
				(callback)=>{
					BkPPS.find({bk:{$regex:regExpBk}, address:{$regex:regExpCity}, year:yearTo}, (err, rep)=>{
						if(rep.length>0) data.yearToQuantity=rep.length;
						else data.yearToQuantity='N/A';
						if(bk!=''&&rep.length>0) data.bk=rep[0].name;
						else if(rep.length>0) data.bk='Все конторы'
						if(city!='') data.city=city;
						data.yearTo=yearTo;
