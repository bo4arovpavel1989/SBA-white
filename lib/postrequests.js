	...
	form.parse(req, function(err, fields, files) {
		...
		if (files.upload.size !== 0 && files.upload.size !== undefined) { /*this is the case, where user uploads photo*/
			async.series([
				...
				(callback)=>{
					//TODO - get last added news and save image, renaming it as ID of DB entry
					News.find({}).sort({date:-1}).limit(1).exec((err,rep)=>{
						if(rep){
							var thumbFileName = __dirname + '/../public/images/news/' + rep[0]._id + '_thumb.jpg';
							var fileName = __dirname + '/../public/images/news/' + rep[0]._id+ '.jpg';
							var htmlFileName = '/images/news/' + rep[0]._id + '.jpg';
							var htmlThumbFileName = '/images/news/' + rep[0]._id + '_thumb.jpg';
							fs.copySync(files.upload.path, fileName);
							fs.unlinkSync(files.upload.path);
							customFunctions.saveThumbImage(400, fileName, thumbFileName);
							News.update({_id:rep[0]._id},{$set:{image:htmlFileName,thumb:htmlThumbFileName}}).exec((err,rep)=>{
								if(err)  callback(new Error('error saving news'))
								else callback();
							});
						}
