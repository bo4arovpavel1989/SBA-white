...
module.exports.getCitiesOfRegion=function(req, res){
	...
	CitiesInfo.find({region:{$regex:region},date:{$gte:dates[0],$lte:dates[1]}}).sort({name:1}).exec((err, rep)=>{
		var data={city:[]};
...
module.exports.getAutocomplete=function(req, res){
	...
	if(list=='city'){
		res.sendFile(__dirname + '/cities/citieslist.json')
	} else res.end();	
};

module.exports.getDataSourcePage=function(req,res){
	let data={layout:'main'};
	res.render('pages/datasourcepage',data);
};

module.exports.getMarketPulsePage=function(req,res){
	let pages=req.query.q;
	var data={news:[]};
	console.log(req.query);
	News.find({}).sort({date:-1}).skip(pages*10).limit(10).exec((err,rep)=>{
		if(rep) data.news=rep;
		if(pages==0){
			data.layout='main';
			res.render('pages/marketpulse',data);
		}	
		else res.render('components/moremarketpulse',data);
	});
};

module.exports.getMoreNews=function(req,res){
	let pages=req.query.q*10;
	var data={news:[]};
	News.find({}).sort({date:-1}).skip(pages).limit(10).exec((err,rep)=>{
		if(rep){
			data.news=rep;
			res.render('pages/marketpulse',data);
		}
...
module.exports.getHistoricalPPS=function(req,res){
	let bkpps=req.params.id;
	let year=req.params.year;
	let month=req.params.month;
	res.sendFile(__dirname + '/bkpps_historical/'+year+'/'+month+'/'+bkpps);
};
...
module.exports.getNewsAddForm=function(req, res){
	res.sendFile('newsaddform.html', {'root':__dirname + '/../views/static/'});
};
