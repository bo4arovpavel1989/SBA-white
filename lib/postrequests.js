...
module.exports.editFormHandler=function(req, res){
	...
	switch(formName){
		...
		break;
		case 'bkpage':
		collection=BookmakerPate;
		breakl
		case 'rate':
		collection=BookmakerRate;
		break;
		case 'social':
		collection=Social;
		break;
...
module.exports.getEditSocialForm=function(req, res){
	let form = new formidable.IncomingForm();
	form.type = 'multipart/form-data';
	form.parse(req, function(err, fields, files) {
		let bk = fields.bk;
		let dates=customFunctions.getFullMonth(fields.year,fields.month);
		Social.findOne({bk:bk,date:{$gte:dates[0],$lte:dates[1]}},(err, rep)=>{
			let data={};
			if(rep) data=rep;
			res.render('forms/editsocialform',data);
		});
	});
};