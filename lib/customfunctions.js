...
module.exports.getAllDatasFromPPS=function(dateArray, data, bk,city,regExpBk,regExpCity,callback){
	...
	function startGettingPPSData(i){
		BkPPS.find({bk:{$regex:regExpBk}, address:{$regex:regExpCity}, begin:{$lte:dateArray[i]},end:{$gte:dateArray[i]}}, 'bk', (err, rep)=>{
				if(rep.length>0) data.datas.push(rep.length);
				else data.datas.push(0);
				if(bk!=''&&rep.length>0) data.bk=rep[0].bk;
