...
var regionArray=[];
	...
	request(options, (err, res, body)=>{
	  if(err){console.log(err);}
		...
		let table=$('.wikitable').get();
		console.log(table.length)
		let lines=table[0].children;
		lines.forEach(line=>{
			//console.log(line);
			try{
			...
			regionObject[region].push(city);
			
			}catch(e){}
		});
		console.log(regionObject);
		regionObject=JSON.stringify(regionObject);
		fs.writeFile('regionList.dat',regionObject);
	  }
	});
