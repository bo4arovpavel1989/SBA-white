...
module.exports.getOffline_region_potential=function(req, res){
	try{
		...
		form.parse(req, function(err, fields, files) {
			...
			async.parallel([
				(callback)=>{
					SalaryInfo.findAll({region:region,date:{$gte:dateBegin,$lte:dateEnd}},(err, rep)=>{
						if(rep) salary=rep.salary;
						callback();
					});
				},
				(callback)=>{
					if(bk=='') CitiesInfo.findOne({name:{$regex:regExpCity},date:{$gte:dateBegin,$lte:dateEnd}},(err, rep)=>{//getting CitiesStats for all bk (popularity of bookmakers itself)
								if(rep) {
									if(bk=='')bk='Все конторы';
									var dataFromDB={
										bkRelation:rep.bkRelation[0],
										bkPopularity:rep.bkPopularity[0],
										salary:rep.salary,
										potentialCoefficient:rep.potentialCoefficient[0]
									};
									data={
										date:fields.year+'-'+fields.month,
										city:city,
										data:dataFromDB
									};
					...
					else 	customFunctions.getCitiesStatsForBk(city,bk,fields,(reply)=>{//getting CitiesStats for certaion bk (popularity of bookmaker)
									data=reply;		
									callback();
							});	
				}
