...
function getVKData(i){
	...
	(err)=>{
		...
		BookmakerPage.update({bk:bks_formatted[i].bk,date:{$gte:dates[0],$lte:dates[1]}},{$set:{'social.vk':objectToWrite}},{upsert:true}).exec((err, rep)=>{	
			Social.update({bk:bks_formatted[i].bk,name:bks_formatted[i].name,date:stringDate},{$set:{dayOfWeek:dayOfWeek,vk:objectToWrite}},{upsert:true}).exec((err,rep2)=>{console.log(rep2)});
			i++;
