...
module.exports.getEditRateForm=function(req,res){
	...
	form.parse(req, function(err, fields, files) {
		let bk = fields.bk;
		let dates=customFunctions.getFullMonth(fields.year,fields.month);
		BookmakerRate.findOne({bk:bk,date:{$gte:dates[0],$lte:dates[1]}},(err, rep)=>{
			let data={};
			if(rep) data=rep;
...
module.exports.editRate=function(req, res){
	let data = req.body;
	let dates=customFunctions.getFullMonth(req.body.year,req.body.month);
	BookmakerRate.update({bk:req.body.bk,date:{$gte:dates[0],$lte:dates[1]}}, data, {upsert:true}).exec((err, rep)=>{
...
module.exports.getEditBkPageForm=function(req, res){
	...
	form.parse(req, function(err, fields, files) {
		let bk = fields.bk;
		let dates=customFunctions.getFullMonth(fields.year,fields.month);
		BookmakerPage.findOne({bk:bk,date:{$gte:dates[0],$lte:dates[1]}},(err, rep)=>{
			let data={};
			if(rep) data=rep;
...
module.exports.editBkPage=function(req, res){
	let data = req.body;
	let dates=customFunctions.getFullMonth(req.body.year,req.body.month);
	BookmakerPage.update({bk:req.body.bk,date:{$gte:dates[0],$lte:dates[1]}}, data, {upsert:true}).exec((err, rep)=>{
		if(!err) res.redirect('/admin')
			else res.end('An error occured: ' + error);
	});
};

module.exports.getEditLineForm=function(req, res){
	let form = new formidable.IncomingForm();
	form.type = 'multipart/form-data';
	form.parse(req, function(err, fields, files) {
		let bk = fields.bk;
		let dates=customFunctions.getFullMonth(fields.year,fields.month);
		Line.findOne({bk:bk,date:{$gte:dates[0],$lte:dates[1]}},(err, rep)=>{
			let data={};
			if(rep) data=rep;
			res.render('forms/editlineform',data);
		});
	});
};