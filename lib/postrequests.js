...
module.exports.getOnlineReputationData=function(req, res){
		try{
			...
			form.parse(req, function(err, fields, files) {
				...
				let datesTo=customFunctions.getFullMonth(fields.yearTo,fields.monthTo);	
				let sources=['intelbet.ru','bookmaker-ratings.ru','sports.ru'];
				let index=customFunctions.getRandomInt(0, sources.length-1);
				async.parallel([
				(callback)=>{
					Comment.find({bk: bk,source:sources[index]},'comment source').sort({date:-1}).limit(5).exec((err, rep)=>{
						data.lastComments=rep;
						callback();
					});
				},
				(callback)=>{
					Feedback.find({bk:bk, date:{$gte:datesFrom[0],$lte:datesFrom[1]}},'source isPositive isNegative isNeutral',function(err, rep){
						data.from=rep;
						callback();
					});
				},
				(callback)=>{
					Feedback.find({bk:bk, date:{$gte:datesTo[0],$lte:datesTo[1]}},'source isPositive isNegative isNeutral',function(err, rep){
						data.to=rep;
						callback();
					});
				}
