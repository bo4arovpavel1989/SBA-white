...
module.exports.getOffline_region_potential=function(req, res){
	try{
		...
		form.parse(req, function(err, fields, files) {
			...
			let city=fields.city;
			if(!city)city='';
			let dateBegin=fields.year+'-'+fields.month+'-01';
			let dateEnd;
			if(Number(fields.month)<12) {
				if(Number(fields.month)<9) dateEnd=fields.year+'-0'+(Number(fields.month)+1)+'-01';
			...
			else 	customFunctions.getCitiesStatsForBk(city,bk,fields,(reply)=>{
							res.render('components/offline_region_potential-table',reply);
					});		
		});
	}catch(e){
		res.send(false);
