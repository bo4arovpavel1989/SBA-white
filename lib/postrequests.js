...
module.exports.getOnlineReputationData=function(req, res){
		try{
			...
			form.parse(req, function(err, fields, files) {
				...
				let sources=['intelbet.ru','bookmaker-ratings.ru','sports.ru','otzovik.com'];
				...
				(callback)=>{
					Feedback.find({bk:bk, date:{$gte:dateFrom,$lte:dateTo}},'source isPositive isNegative isNeutral',function(err, rep){
						data.feedback=rep;
						callback();
					});
				}
