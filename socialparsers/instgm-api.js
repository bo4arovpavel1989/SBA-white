  ...
  .then(function (body) {
	  console.log('start parsing');
		...
		try{
			...
			obj.sbcrb=obj.sbcrb.replace(/\s/ig, '');
		...
		BookmakerPage.update({bk:bks_formatted[i].bk,date:{$gte:dates[0],$lte:dates[1]}},{$set:{'social.insta':obj}},{upsert:true}).exec((err, rep)=>{	
			Social.update({bk:bks_formatted[i].bk,name:bks_formatted[i].name,date:stringDate},{$set:{dayOfWeek:dayOfWeek,instgm:obj}},{upsert:true}).exec();
			i++;
