...
var getFullMonth = function(year, month){
	...
	year=Number(year);
	month=Number(month);
	var dateBegin = new Date(Date.UTC(year, month-1, 1)); //utc month from 0 to 11
	var dateMiddle = new Date(Date.UTC(year,month-1, 15));
	var dateEnd = new Date(Date.UTC(year,month, 0));
	console.log(dateBegin);
	return [getStringDateAndTime(dateBegin)[0],getStringDateAndTime(dateEnd)[0],getStringDateAndTime(dateMiddle)[0]];
};
...
var calculateMiddleMonthes=function (date1, date2, options){
	...
	if( (diff>0&&options.unlimited) || (diff>0&&diff<370*24*60*60*1000)){ //i get unlimited data or data for a year length
