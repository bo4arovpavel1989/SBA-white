...
module.exports.getOffline_table=function(req, res){
	try{
		...
		form.parse(req, function(err, fields, files) {
			...
			async.parallel([
				(callback)=>{
					BkPPS.find({bk:{$regex:regExpBk}, address:{$regex:regExpCity}, begin:{$lte:dateFrom},end:{$gte:dateFrom}}, 'name', (err, rep)=>{
				...
				(callback)=>{
					BkPPS.find({bk:{$regex:regExpBk}, address:{$regex:regExpCity},  begin:{$lte:dateTo},end:{$gte:dateTo}}, 'name', (err, rep)=>{
...
module.exports.getOffline_graph=function(req, res){
	try{
		...
		form.parse(req, function(err, fields, files) {
			...
			customFunctions.getAllDatasFromPPS(dateArray, data,bk,city,regExpBk,regExpCity,(reply)=>{
				console.log(reply);
				res.send(reply);
			});
		});
